...
def g_theta_local(model, sampleX, sample_label, theta, pre_v, nclasses, ratio = 0.02, tolerance = 0.0001):
...
def attack(model, tdata, tlabel, x0, y0, nclasses, index, step = 0.2, beta = 0.001, iterations = 1000):    
    q = 20    
    nf = len(x0)    
    best_theta, g_theta, dis = None, float('inf'), float('inf')
    for ii in range(len(tlabel)):
        if predict(model, tdata[ii], nclasses) != y0:
            theta = tdata[ii] - x0
            initial_lbd = 1.0            
