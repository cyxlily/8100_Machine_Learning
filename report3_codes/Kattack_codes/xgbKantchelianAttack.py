...
def main(args):
	...
	os.chdir('{}/report3_results/Kattack_results'.format(home))
	with open('{}_Kan_{}_ave.txt'.format(d_name, m_name), 'w') as f:
		f.write('attacked {}/{} points, average linf distortion: {}, average time:{}'.format(counter, num_attacks, avg_dist/counter, (time.time()-global_start)/counter))  
	f.close()
	print('{} is done'.format(d_name))

if __name__ == '__main__':
	parser = argparse.ArgumentParser()
	parser.add_argument('-dn', '--data_name',type=str, help = 'name of the data set')
	parser.add_argument('-mn', '--model_name',type=str, choices=['xgb','rxgb'], help = 'natural or robust model')
